...
import unittest
import yaml
from encrypteddict import decrypter, DecryptionError, UnsupportedEncryptionMethod
...
class TestDecrypt(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for key in self.import_keys:
            with self.keyfile(key) as fp:
                self.ctx.import_(fp)
        self.test_decrypter = decrypter(gpg_home=self._gpghome)
    ...
    def test_decrypt_with_no_dict_raises_exception(self):
        with self.assertRaises(TypeError):
            self.test_decrypter.decrypt_all()
